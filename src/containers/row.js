    ...
    render() {
      ...
      return (
        ...
        < td > <div>{
          ...
          this.props.result.qualification
        } < /td> < td > {
          this.props.result.age
        } < /td> < td > {
          this.props.result.applicationProcedure
        } < /td> < td > {
          this.props.result.selectionProcess
        } < /td> < td > {
          this.props.result.source
        } < /td> < td > < Time value = {
          this.props.result.lastDate
        }
        format = "YYYY/MM/DD" / > < /td> < td > < Time value = {
          this.props.result.notificationDate
        }
        format = "YYYY/MM/DD" / > < /td> < td > {
          this.props.result.source
        } < /td> < td > {
          this.props.result.duration
        } < /td> < td > {
          this.props.result.contactDetails
        } < /td> < td > {
          tags
        } < /td> < td > < Time value = {
          this.props.result.entranceExamDate
        }
        format = "YYYY/MM/DD" / > < /td> < td > {
